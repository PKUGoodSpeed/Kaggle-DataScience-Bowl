...
if __name__ == '__main__':
    ...
    train_x, train_y = ip.get_batch_resized(train_idx=train_idx)
    if C['augment']:
        train_x, train_y = ip.augment(train_x, train_y)
    valid_x, valid_y = ip.get_batch_resized(train_idx=valid_idx)
    resn.fit(x=train_x, y=train_y, valid_set=(valid_x, valid_y), 
    check_file='resz_weights_test.h5', **C['fit_kargs'])

    resn.load_model('./checkpoints/resz_weights_test.h5')
